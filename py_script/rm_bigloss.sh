...
#import json
# def readfile():
#     f = open("format_uniq_mis_ambigu.tmp2", "rb") #二进制格式读文件
#     count=0
#     while True:
#         line = f.readline()
#         count+=1
...
#                 c=json.loads(line)
#                 with open('fix_format_uniq_mis_ambigu.tmp2', 'a') as dd:
...
dct={}
def select_false():
  data=[json.loads(x) for x in open('fix_format_uniq_mis_ambigu.tmp2')]
  for d in data:
  ...
  for d in data2:
    exist=dct.get(d['candidates'][1]['output'])
    print(exist)
    if d['candidates'][1]['source']=='ambiguous_pair' and exist is not None:
      continue
    with open('/cpfs01/rl/RM/labeler_0607/fix_merge_from_num_calc_v2.json', 'a') as dd:

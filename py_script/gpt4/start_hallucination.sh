...
#for i in `seq -w $((jobnum-1)) -1 0` ; do (
for i in `seq -f "%02g" 0 $((jobnum-1))` ; do (
  echo ${infn}_${i}
        #python gpt4_client.py ${infn} $outfn $version
        #python gpt4_client_askzhendui.py ${infn}_${i} $outfn $version
        #python gpt4_client.py ${infn}_${i} $outfn $version
        #python gpt4_answer.py ${infn}_${i} $outfn $version
        #python gpt4_client_asklogic.py ${infn}_${i} $outfn $version
        python gpt4_hallucination.py ${infn}_${i} $outfn $version
        #python gpt4_client_asktruth.py ${infn}_${i} $outfn $version

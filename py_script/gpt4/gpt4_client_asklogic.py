...
def process_with_retry(grpc_client: GrpcClient, request, query, time_out, n_times, id_key):
    ...
    for i in range(n_times):
        try:
            ...
            #逻辑打分 
            prompt="[System]\nWe would like to request your feedback on the performance of the response of the assistant to the user question displayed below.\nIn the feedback, I want you to rate the quality of the response in the following dimension to the given scoring rubric:\n逻辑性: 回复的文本是否逻辑通顺,包括逻辑的连贯性、逻辑的严谨性、逻辑的完整性,考虑其推理步骤的普遍适用并避免逻辑矛盾。同时包括考虑编码和数学问题的边缘情况,没有未考虑到的反例；考虑是否在问题的限定下已最简单高效的方式推理,时间复杂度低且无多余步骤。\nScore 1：逻辑完全不连贯、模型的最终答案完全不正确且缺乏推理过程或过程完全错误、绝大部分的过程冗余低效\nScore 2：最终结果正确但推理过程绝大部分错误或无逻辑性、最终结果错误但推理过程有明显问题（包括但不限于过程不完整、过程大部分冗余）\nScore 3：满足以下情况的判定为3分：最终答案错误但有推理过程，且推理过程大部分均是正确的，最终可从推理过程中提炼正确答案；最终答案正确，但无推理过程；大部分正确没有考虑到一些边界情况\nScore 4：模型的最终答案或过程包含瑕疵，这些错误很容易纠正，不会对其整体正确性产生重大影响，或者大部分正确没有考虑到一些边界情况，允许有少部分多余的步骤可优化\nScore 5：最终结果和过程均正确且高效，在逻辑上是完美的，考虑了所有潜在的边缘情况。\nYou will be given two assistants' answer.\nPlease give feedback on each assistant's response. Also, provide each assistant with a score on a scale of 1 to 5 for the scoring dimension, where a higher score indicates better overall performance. Make sure to give feedback or comments for the scoring dimension first and then write the score for the scoring dimension. Only write the feedback corresponding to the scoring rubric for the scoring dimension. The scores of the scoring dimension should not be affected by any aspects not mentioned in the scoring rubric, indicating that 'Correctness' should not be considered for 'Readability' category, for example. \nMake sure the feedback strictly follow this format:\nEvaluation evidence: <your explanation of answer A here>\nScore: <score of answer A>\nEvaluation evidence: <your explanation of answer B here>\nScore: <score of answer B>\n[User Question]\n{quest}\n\n[The Start of Assistant’s Answer A]\n{ansa}\n[The End of Assistant’s Answer A]\n\n[The Start of Assistant’s Answer B]\n{ansb}\n[The End of Assistant’s Answer B]"
            #请求针对性pair打分         

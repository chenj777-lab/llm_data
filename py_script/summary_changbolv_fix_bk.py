...
#    date = date.rstrip(os.linesep)
   ...
   # m = rep.match(date)
    #res = datetime.datetime(int(m.group(1)),int(m.group(2)),int(m.group(3)),int(m.group(4)),int(m.group(5)),int(m.group(6)))
        ...
        for text in f:
            ...
            for user_index in range(kLiveInfoLen,texts_len):
                ...
                if calc_online_time >= user_end_timestamp_int and calc_online_time - duration < user_start_timestamp_int:
                    '''
                    if calc_online_time == live_start_timestamp_int + duration:
                        current_index = math.ceil((calc_online_time - live_start_timestamp_int)/duration) -1
                    elif calc_online_time == user_start_timestamp_int:
                        current_index = math.ceil((calc_online_time+1 - live_start_timestamp_int)/duration) -1
                    '''
               ...
               #     current_index = math.ceil((user_start_timestamp_int - live_start_timestamp_int)/duration) -1
                   # current_index +=1
                    if current_index>=live_time_len:
                        print(current_index,',',live_time_len)
                        print(live_start_timestamp_int,',',live_end_timestamp_int,',',user_start_timestamp_int,',',user_end_timestamp_int)
                    user_online_time_list[current_index] += 1
